...
def main():
    ...
    ap.add_argument("--csv", required=True, help="Input per-second dataset CSV (15-col or 18-col mobility version)")
    ...
    required_base = {
        ...
        "rsrp", "sinr_db_reported", "sinr_linear", "sinr_db_calc", "distance_ue",
        "trigger_a3", "trigger_kpi", "gap_db"
    }
    missing = required_base - set(df.columns)
    if missing:
        raise ValueError(f"Missing required columns: {sorted(missing)}")

    has_mobility = all(c in df.columns for c in ["speed_ue", "dx_ue", "dy_ue"])
    ...
    if has_mobility:
        feat_cols += ["speed_ue", "dx_ue", "dy_ue"]
    ...
    print("Detected mobility columns:", has_mobility)
