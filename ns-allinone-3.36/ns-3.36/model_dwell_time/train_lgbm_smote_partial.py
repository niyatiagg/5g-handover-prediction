...
def main():
    parser = argparse.ArgumentParser(description="LightGBM with training-only partial SMOTE")
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.12,
        help="Target minority/majority ratio after SMOTE on training only, e.g. 0.10 or 0.15",
    )
    parser.add_argument(
        "--k-neighbors",
        type=int,
        default=5,
        help="SMOTE k_neighbors",
    )
    ...
    # after SMOTE, use neutral class weighting first
    ...
    metrics = {
        ...
        "n_train_positive_original": int(y_train.sum()),
        "n_test_positive": int(y_test.sum()),
        "positive_rate_train_original": float(y_train.mean()),
        "positive_rate_test": float(y_test.mean()),
        "n_train_resampled": int(len(y_train_res)),
        "n_train_positive_resampled": int(np.sum(y_train_res)),
        "positive_rate_train_resampled": float(np.mean(y_train_res)),
    ...
    preds_path = outdir / "lgbm_smote_test_predictions.csv"
    preds_df.to_csv(preds_path, index=False)

    sweep_df = compute_threshold_metrics(preds_df, prob_col="y_prob", y_col="y_true")
    sweep_path = outdir / "lgbm_smote_threshold_sweep.csv"
    sweep_df.to_csv(sweep_path, index=False)

    metrics_path = outdir / "lgbm_smote_metrics.json"
