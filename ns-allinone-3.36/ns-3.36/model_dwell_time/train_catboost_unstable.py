...
def main():
    parser = argparse.ArgumentParser(description="Train CatBoost for unstable_15 prediction")
    ...
    preds_path = outdir / "catboost_test_predictions.csv"
    preds_df.to_csv(preds_path, index=False)

    sweep_df = compute_threshold_metrics(preds_df, prob_col="y_prob", y_col="y_true")
    sweep_path = outdir / "catboost_threshold_sweep.csv"
    sweep_df.to_csv(sweep_path, index=False)

    metrics_path = outdir / "catboost_metrics.json"
    with open(metrics_path, "w") as f:
        json.dump(metrics, f, indent=2)

    try:
        imp_df = pd.DataFrame(
            {"feature": feature_cols, "importance": clf.get_feature_importance()}
        ).sort_values("importance", ascending=False).reset_index(drop=True)
        imp_path = outdir / "catboost_feature_importance.csv"
