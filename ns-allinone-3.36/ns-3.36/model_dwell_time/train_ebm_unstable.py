...
def main():
    parser = argparse.ArgumentParser(description="Train EBM for unstable_15 prediction")
    ...
    preds_path = outdir / "ebm_test_predictions.csv"
    preds_df.to_csv(preds_path, index=False)

    sweep_df = compute_threshold_metrics(preds_df, prob_col="y_prob", y_col="y_true")
    sweep_path = outdir / "ebm_threshold_sweep.csv"
    sweep_df.to_csv(sweep_path, index=False)

    metrics_path = outdir / "ebm_metrics.json"
    with open(metrics_path, "w") as f:
        json.dump(metrics, f, indent=2)

    # simple global importance export
    try:
