...
def main():
    parser = argparse.ArgumentParser(description="Train Random Forest for unstable_15 prediction")
    ...
    preds_path = outdir / "rf_test_predictions.csv"
    preds_df.to_csv(preds_path, index=False)

    sweep_df = compute_threshold_metrics(preds_df, prob_col="y_prob", y_col="y_true")
    sweep_path = outdir / "rf_threshold_sweep.csv"
    sweep_df.to_csv(sweep_path, index=False)

    metrics_path = outdir / "rf_metrics.json"
    ...
    try:
        ...
        imp_path = outdir / "rf_feature_importance.csv"
