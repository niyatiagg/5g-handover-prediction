 ...
 * distance_ue,speed_ue,dx_ue,dy_ue,trigger_a3,trigger_kpi,gap_db
 *
 * Definitions:
 ...
 * - gap_db (last col): (candidate_rsrp - serving_rsrp) in dB
 * - sinr_linear (col10): raw SINR (linear) from UE PHY ReportCurrentCellRsrpSinr
 * - speed_ue: sampled UE speed magnitude derived from consecutive sampled positions
 * - dx_ue, dy_ue: sampled 1-second x/y displacement components (meters)
...
struct PerUeState
{
  ...
  // Mobility context at sampling resolution
  Vector prevPos = Vector (0,0,0);
  double prevSampleTime = std::numeric_limits<double>::quiet_NaN ();
  ...
  for (const auto &kv : g_imsiToIdx)
    {
      ...
      // Mobility context from sampled positions
      double speedUe = std::numeric_limits<double>::quiet_NaN ();
      double dxUe = std::numeric_limits<double>::quiet_NaN ();
      double dyUe = std::numeric_limits<double>::quiet_NaN ();
      if (S.hasPrevPos && std::isfinite (S.prevSampleTime))
        {
          const double dt = t - S.prevSampleTime;
          if (dt > 0.0)
            {
              dxUe = S.pos.x - S.prevPos.x;
              dyUe = S.pos.y - S.prevPos.y;
              speedUe = std::sqrt (dxUe * dxUe + dyUe * dyUe) / dt;
            }
        }
      S.prevPos = S.pos;
      S.prevSampleTime = t;
